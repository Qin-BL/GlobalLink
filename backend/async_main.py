...
from app.api.async_api import async_api_router
from app.core.config import settings
from app.db.async_init_db import init_db
...
# 包含异步API路由
app.include_router(async_api_router, prefix=settings.API_V1_STR)
...
async def shutdown_event():
    """应用关闭事件（异步版本）"""
    logger.info("关闭异步版本应用...")
    
    try:
        # 关闭数据库连接
        await async_engine.dispose()
        
        logger.info("异步应用关闭完成")
...
async def read_root():
    """根端点（异步版本）"""
    return {
        "message": "Welcome to GlobalLink API (Async Version)",
        "version": "2.0.0",
        "docs": "/docs",
        "redoc": "/redoc"
    }
